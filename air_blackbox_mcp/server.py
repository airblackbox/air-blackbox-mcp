...
async def add_trust_layer(code: str, framework: str = "") -> str:
    ...
    Supports: langchain, crewai, autogen, openai, haystack (rag), adk, claude.
    ...
    if fw not in TRUST_LAYER_TEMPLATES:
        return json.dumps({
            ...
            "tip": "Try one of: langchain, crewai, autogen, openai, haystack, adk, claude",
        }, indent=2)
...
async def generate_compliance_report(code: str) -> str:
    ...
    lines.append(f"\n**Generated by**: AIR Blackbox MCP v{__version__}")
...
async def compliance_history(action: str = "list") -> str:
    ...
    try:
        if action.lower() == "trend":
            ...
            # Full audit trail: complete history plus trend and latest score
            result = {
                "history": get_history(),
                "trend": get_trend(),
